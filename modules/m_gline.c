...
#ifdef GLINE_VOTING
static int check_majority_gline(const struct Client *, const char *, const char *, const char *);

static void add_new_majority_gline(const struct Client *, const char *, const char *, const char *);
#endif /* GLINE_VOTING */
...
mo_gline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, ":%s NOTICE %s :GLINE disabled", me.name, source_p->name);
		return;
	}
	...
	if(check_majority_gline(source_p, user, host, reason) == GLINE_ALREADY_VOTED)
	{
		sendto_one(source_p,
	...
	ilog(L_TRACE, "#gline for %s@%s [%s] requested by %s!%s@%s",
	     user, host, reason, source_p->name, source_p->username, source_p->host);
#else
...
static void
me_gline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	do_sgline(client_p, source_p, parc, parv, 0);
}

static void
do_sgline(struct Client *client_p, struct Client *source_p, int parc, char *parv[], int prop)
{
	...
	DLINK_FOREACH(ptr, gdeny_items.head)
	{
		...
		if(match(conf->name, source_p->servptr->name) &&
		   match(aconf->user, source_p->username) && match(aconf->host, source_p->host))
		{
	...
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "Blocked G-Line %s requested on [%s@%s] [%s]",
				     get_oper_name(source_p), user, host, reason);
	...
	if(var_offset & GDENY_REJECT)
	{
		...
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "Rejected G-Line %s requested on [%s@%s] [%s]",
					     get_oper_name(source_p), user, host, reason);
	...
	if(ConfigFileEntry.glines)
	{
		...
		if(IsClient(source_p))
		{
			...
			{
				int bitlen = strtol(++p, NULL, 10);
				int min_bitlen = strchr(host,
							':') ? ConfigFileEntry.
					gline_min_cidr6 : ConfigFileEntry.gline_min_cidr;

				if(bitlen < min_bitlen)
				{
					sendto_realops_flags(UMODE_ALL, L_ALL,
							     "%s!%s@%s on %s is requesting "
							     "a GLINE with a CIDR mask < %d for [%s@%s] [%s]",
							     source_p->name, source_p->username,
							     source_p->host,
							     source_p->servptr->name, min_bitlen,
							     user, host, reason);
					return;
				}
			}
		   ...
		   (irccmp(gline_pending_ptr->host, host) == 0))
		{
			...
			if(gline_pending_ptr->oper_user2[0] != '\0')
			{
				...
				if(((irccmp(gline_pending_ptr->oper_user2, source_p->username) == 0)
				   ...
				   ||
				   (irccmp(gline_pending_ptr->oper_server2, source_p->servptr->name)
				    == 0))
				{
					return (GLINE_ALREADY_VOTED);
				}
...
mo_ungline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(!ConfigFileEntry.glines)
	{
		sendto_one(source_p, ":%s NOTICE %s :UNGLINE disabled", me.name, source_p->name);
		return;
	}
