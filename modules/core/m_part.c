 ...
 *  USA
 *
 *  $Id: m_part.c 291 2006-02-18 00:50:37Z jon $
 */
...
part_one_client(struct Client *client_p, struct Client *source_p, char *name, char *reason)
{
	...
	if(reason[0] && (!MyConnect(source_p) ||
			 ((can_send(chptr, source_p, ms, reason, NO) > 0 &&
...
m_part(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(*parv[1] == '\0')
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name, source_p->name, "PART");
		return;
	}
