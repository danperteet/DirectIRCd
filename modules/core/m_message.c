 ...
 *  USA
 *
 *  $Id: m_message.c 493 2007-07-07 17:45:14Z jon $
 */
		  ...
		  struct Client *source_p, struct Channel *chptr, int flags, char *text)
{
	...
	if(flags & CHFL_VOICE)
	{
		type = CHFL_VOICE | CHFL_HALFOP | CHFL_CHANOP | CHFL_PROTECTED | CHFL_OWNER;
		c = '+';
	}
#ifdef HALFOPS
	else if(flags & CHFL_HALFOP)
	{
		type = CHFL_HALFOP | CHFL_CHANOP | CHFL_PROTECTED | CHFL_OWNER;
		c = '%';
	}
#endif
#ifdef CHANAQ
	...
	{
		type = CHFL_CHANOP | CHFL_PROTECTED | CHFL_OWNER;
		c = '@';
	}
	else if(flags & CHFL_PROTECTED)
	{
		type = CHFL_PROTECTED | CHFL_OWNER;
		c = '&';
	}
	else
	{
	...
	if(MyClient(source_p))
	{
		...
		sendto_channel_local_butone(source_p, type, YES, chptr, ":%s!%s@%s %s %c%s :%s",
...
{
	if(IsRegOnly(target_p) && !IsRegNick(source_p) && !IsPrivileged(source_p))
	{
		if(p_or_n != NOTICE)
	...
	if(MyClient(target_p))
	{
			...
			else
			{
				    ...
				    ConfigFileEntry.caller_id_wait) < CurrentTime)
				{
					...
					sendto_one(target_p, form_str(RPL_UMODEGMSG),
						   ...
						   get_client_name(source_p, IsOper(target_p) ?
								   HIDE_IP : HIDE_REALHOST),
						   notsoft ? "+g" : "+G");
		...
		else
		{
			 ...
			 * and flooding    -- fl */
			if(!MyClient(source_p) || IsOper(source_p) || IsServices(source_p) ||
	       ...
	       struct Client *source_p, char *nick, char *text)
{
	...
	if(!IsOper(source_p) && !IsServices(source_p))
	{
		sendto_one(source_p, form_str(ERR_NOPRIVILEGES),
			   ID_or_name(&me, client_p), ID_or_name(source_p, client_p));
		return;
 ...
 *
 */
static struct Client *find_userhost(char *user, char *host, int *count)
{
	struct Client *c2ptr;
