 ...
 *  USA
 *
 *  $Id: m_squit.c 147 2005-12-26 02:52:10Z jon $
 */
...
mo_squit(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NOSUCHSERVER), me.name, source_p->name, server);
		return;
	}

	if(IsServices(target_p) || !IsRouting(source_p))
	{
...
ms_squit(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(MyConnect(target_p))
	{
		...
		ilog(L_TRACE, "SQUIT From %s : %s (%s)", parv[0], target_p->name, comment);
