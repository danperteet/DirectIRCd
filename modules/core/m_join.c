 ...
 *  USA
 *
 *  $Id: m_join.c 469 2007-03-17 02:59:20Z jon $
 */
...
m_join(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name, source_p->name, "JOIN");
		return;
	}
	...
	for(chan = strtoken(&p, chan_list, ","); chan; chan = strtoken(&p, NULL, ","))
	{
		...
		/* Allow opers to bypass most channel name checks */
		...
		if(!IsExemptResv(source_p) &&
		   ...
		   (!(resv_p = hash_find_resv(chan)) == ConfigChannel.restrict_channels))
		{
			if(resv_p != NULL)
				sendto_one(source_p, form_str(ERR_CHANBANREASON),
					   me.name, source_p->name, chan,
					   resv_p->reason ? resv_p->reason : "Reserved Channel");
			else
				sendto_one(source_p, form_str(ERR_CHANBANREASON),
		...
		if((i = can_join(source_p, chptr, key)))
		{
			if(i == ERR_NEEDREGGEDNICK)
				sendto_one(source_p, form_str(ERR_NEEDREGGEDNICK), me.name,
					   source_p->name, chptr->chname, "join");
			/* we use ERR_CHANBANREASON for the SSL response due to wider client support */
			else if(i == ERR_CHANBANREASON)
				sendto_one(source_p, form_str(ERR_CHANBANREASON), me.name,
					   source_p->name, chptr->chname, "SSL is required");
			else
