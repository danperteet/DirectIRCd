 ...
 *  USA
 *
 *  $Id: m_kill.c 327 2006-02-25 21:36:54Z jon $
 */
...
const char *_version = "$Revision: 327 $";
#endif
...
mo_kill(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(*user == '\0')
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name, source_p->name, "KILL");
		return;
	}
	...
	if((target_p = find_client(user)) == NULL)
	{
		...
		if((target_p = get_history(user,
					   (time_t) ConfigFileEntry.kill_chase_time_limit)) == NULL)
		{
	...
	if(IsServer(target_p) || IsMe(target_p) || IsServices(target_p) || IsNetAdmin(target_p))
	{
		sendto_one(source_p, form_str(ERR_CANTKILLSERVER), me.name, source_p->name);
		return;
	}
	...
	if(MyConnect(target_p))
	{
		if(IsServer(source_p))
		{
			...
			if((IsHidden(source_p) || ConfigServerHide.hide_servers)
			   && !IsOper(target_p))
				sendto_one(target_p, ":%s KILL %s :%s", me.name, target_p->name,
					   reason);
			else
				sendto_one(target_p, ":%s KILL %s :%s",
