 ...
 *  USA
 *
 *  $Id: m_sjoin.c 403 2006-06-14 05:04:42Z jon $
 */
...
const char *_version = "$Revision: 403 $";
#endif
...
static void introduce_lazy_link_clients(struct Client *, struct Client *, struct Channel *);
...
ms_sjoin(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	 ...
	 * ~&@%+", and a space.
	 */
	if(buflen >= (IRCD_BUFSIZE - IRCD_MAX(NICKLEN, IDLEN) - 2 - 5 - 1))
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
	...
	*mbuf++ = '+';
	...
	while(*s)
	{
		 ...
		 * sometimes.
		 */
		if(target_p == NULL || target_p->from != client_p || !IsClient(target_p))
		{
			goto nextnick;
		}
		...
		{
			if(fl & (CHFL_OWNER | CHFL_PROTECTED | CHFL_CHANOP | CHFL_HALFOP))
				fl = CHFL_DEOPPED;
			else
				fl = 0;
		}
	...
	*mbuf = '\0';
	...
	if(HasID(source_p) && !keep_our_modes)
	{
		if(dlink_list_length(&chptr->banlist) > 0)
			remove_ban_list(chptr, client_p, &chptr->banlist, 'b', NOCAPS);

		if(dlink_list_length(&chptr->exceptlist) > 0)
			remove_ban_list(chptr, client_p, &chptr->exceptlist, 'e', CAP_EX);

		if(dlink_list_length(&chptr->invexlist) > 0)
			remove_ban_list(chptr, client_p, &chptr->invexlist, 'I', CAP_IE);
...
remove_our_modes(struct Channel *chptr, struct Client *source_p)
{
	...
	remove_a_mode(chptr, source_p, CHFL_PROTECTED, 'a');
#endif
 ...
 */
static void
remove_ban_list(struct Channel *chptr, struct Client *source_p, dlink_list * list, char c, int cap)
{
	char lmodebuf[MODEBUFLEN];
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, list->head)
	{
		...
		*mbuf++ = c;
		cur_len += plen;
		pbuf += ircsprintf(pbuf, "%s!%s@%s ", banptr->name, banptr->username, banptr->host);
		++count;
	...
	*mbuf = *(pbuf - 1) = '\0';
	sendto_channel_local(ALL_MEMBERS, NO, chptr, "%s %s", lmodebuf, lparabuf);
	sendto_server(source_p, NULL, chptr, cap, CAP_TS6, NOFLAGS, "%s %s", lmodebuf, lparabuf);
}
 ...
 */
static void
introduce_lazy_link_clients(struct Client *client_p, struct Client *target_p, struct Channel *chptr)
{
	...
	DLINK_FOREACH(m, serv_list.head)
	{
		...
		if(!(target_p->lazyLinkClientExists & lclient_p->localClient->serverMask))
		{
