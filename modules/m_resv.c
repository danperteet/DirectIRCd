 ...
 *  USA
 *
 *  $Id: m_resv.c 505 2007-07-16 16:47:50Z jon $
 */
	...
	if(target_server != NULL)
	{
		...
		if(tkline_time != 0)
			...
			sendto_match_servs(source_p, target_server, CAP_CLUSTER,
					   "RESV %s %s :%s", target_server, resv, reason);
 ...
 */
static void
ms_resv(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(find_matching_name_conf(ULINE_TYPE, source_p->servptr->name,
				   source_p->username, source_p->realhost, SHARED_RESV))
