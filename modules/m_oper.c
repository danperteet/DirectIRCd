 ...
 *  USA
 *
 *  $Id: m_oper.c 455 2006-09-08 20:53:08Z jon $
 */
...
const char *_version = "$Revision: 455 $";
#endif
...
m_oper(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(svsnoop)
	{
		...
		failed_oper_notice(source_p, name, "This server is in NOOP mode");
	...
	if(match_conf_password(password, aconf))
	{
		...
		ilog(L_TRACE, "OPER %s by %s!%s@%s",
		     name, source_p->name, source_p->username, source_p->realhost);
