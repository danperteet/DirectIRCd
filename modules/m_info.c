 ...
 *  USA
 *
 *  $Id: m_info.c 493 2007-07-07 17:45:14Z jon $
 */
...
static const struct InfoStruct info_table[] = {
	 ...
	 "G-Line log file"},
	{
	 "cycle_on_hostchange",
	 OUTPUT_BOOLEAN_YN,
	 &ConfigChannel.cycle_on_hostchange,
	 "Enables the cycling of channels by clients changing their hostname"},
	{
...
send_info_text(struct Client *source_p)
{
	...
	while(*text)
	{
		...
		sendto_one(source_p, form_str(RPL_INFO), source, target, line);
	}
	...
	sendto_one(source_p, form_str(RPL_ENDOFINFO), me.name, source_p->name);
}
...
send_birthdate_online_time(struct Client *source_p)
{
	if(!MyClient(source_p) && IsCapable(source_p->from, CAP_TS6) && HasID(source_p))
	{
		sendto_one(source_p, ":%s %d %s :On-line since %s",
			   me.id, RPL_INFO, source_p->id, myctime(me.firsttime));
	}
	else
	{
		sendto_one(source_p, ":%s %d %s :On-line since %s",
...
#endif
	sendto_one(source_p, form_str(RPL_INFO), from, to, "");
}
