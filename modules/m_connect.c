 ...
 *  USA
 *
 *  $Id: m_connect.c 147 2005-12-26 02:52:10Z jon $
 */
...
mo_connect(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NOPRIVS), me.name, source_p->name, "connect");
		return;
	}
	...
	else if((conf = find_matching_name_conf(SERVER_TYPE, NULL, NULL, parv[1], 0)) != NULL)
		aconf = (struct AccessItem *) map_to_conf(conf);
	 ...
	 */
	ilog(L_TRACE, "CONNECT From %s : %s %s", source_p->name, parv[1], parv[2] ? parv[2] : "");
	...
	if(serv_connect(aconf, source_p))
	{
		if(!ConfigServerHide.hide_server_ips && IsAdmin(source_p))
			sendto_one(source_p, ":%s NOTICE %s :*** Connecting to %s[%s].%d",
				   me.name, source_p->name, aconf->host, conf->name, aconf->port);
		else
