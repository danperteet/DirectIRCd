 ...
 *  USA
 *
 *  $Id: m_llnick.c 147 2005-12-26 02:52:10Z jon $
 */
 ...
 */
static void
ms_llnick(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	char *nick;
	char *nick_old = NULL;
	...
	if(!IsCapable(client_p, CAP_LL) || !IsCapable(client_p, CAP_HUB))
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "*** LLNICK requested from non LL server %s", client_p->name);
		return;
	}
