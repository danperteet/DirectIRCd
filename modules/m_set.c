 ...
 *  USA
 *
 *  $Id: m_set.c 403 2006-06-14 05:04:42Z jon $
 */
...
quote_jfloodtime(struct Client *source_p, int newval)
{
	...
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "%s has changed JFLOODTIME to %i", source_p->name, newval);
...
quote_jfloodcount(struct Client *source_p, int newval)
{
	...
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "%s has changed JFLOODCOUNT to %i", source_p->name, newval);
...
mo_set(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(parc > 1)
	{
		...
		for(i = 0; set_cmd_table[i].handler; i++)
		{
			if(irccmp(set_cmd_table[i].name, parv[1]) == 0)
			{
				...
				if(set_cmd_table[i].wants_int && (parc > 2))
				{
					if(intarg)
					{
						if(irccmp(intarg, "yes") == 0
						   || irccmp(intarg, "on") == 0)
							newval = 1;
						else if(irccmp(intarg, "no") == 0
							|| irccmp(intarg, "off") == 0)
							newval = 0;
						else
							newval = atoi(intarg);
		 ...
		 */
		sendto_one(source_p, ":%s NOTICE %s :Variable not found.", me.name, source_p->name);
		return;
	}
