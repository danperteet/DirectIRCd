 ...
 *  USA
 *
 *  $Id: m_who.c 493 2007-07-07 17:45:14Z jon $
 */
#include "stdinc.h"
#include "tools.h"
#include "common.h"
...
m_who(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(IsChanPrefix(*mask))
	{
		...
		sendto_one(source_p, form_str(RPL_ENDOFWHO), from, to, mask);
		return;
	}
	   ...
	   IsClient(target_p) && (!server_oper || IsOper(target_p)))
	{
		...
		DLINK_FOREACH(lp, target_p->channel.head)
		{
			...
			if((PubChannel(chptr) && !IsServices(target_p) && !IsHideChannels(target_p))
			   || IsMember(source_p, chptr) || IsAdmin(source_p))
				break;
		}

		if(lp != NULL)
			do_who(source_p, target_p, chptr->chname,
			       get_member_status(lp->data, NO, NO));
		else
	...
	DLINK_FOREACH(ptr, chptr->members.head)
	{
		...
		if((mask == NULL) ||
		   ...
		   ((!ConfigServerHide.hide_servers || IsOper(source_p)) &&
		    match(mask, target_p->servptr->name)) || match(mask, target_p->info))
		{
			do_who(source_p, target_p, NULL, "");
		  ...
		  const char *chname, int member, int server_oper)
{
	...
	DLINK_FOREACH_SAFE(ptr, ptr_next, chptr->members.head)
	{
		...
		if(member || IsAdmin(source_p) ||
		   (!IsInvisible(target_p) && !IsServices(target_p) && !IsHideChannels(target_p)))
		{
			if(server_oper && !IsOper(target_p))
				continue;
			do_who(source_p, target_p, chname, get_member_status(ms, NO, NO));
		}
	}
}
...
*/
static void
ms_who(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	/* If its running as a hub, and linked with lazy links
	 * then allow leaf to use normal client m_who()
	 * other wise, ignore it.
	 */
	if(IsClient(source_p) && ServerInfo.hub && IsCapable(client_p->from, CAP_LL))
