 ...
 *  USA
 *
 *  $Id: m_etrace.c 469 2007-03-17 02:59:20Z jon $
 */
 ...
 */
static void
mo_etrace(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
		...
		if(full_etrace)
		{
			if(ConfigFileEntry.hide_spoof_ips)
				sendto_one(source_p, FORM_STR_RPL_ETRACE_FULL,
					   ...
					   IsIPSpoof(target_p) ? "<hidden>" : target_p->
					   client_server, target_p->info);
			else
				sendto_one(source_p, FORM_STR_RPL_ETRACE_FULL,
					   me.name,
					   source_p->name,
					   ...
					   target_p->client_server, target_p->info);
		}
		else
		{
			if(ConfigFileEntry.hide_spoof_ips)
				...
				sendto_one(source_p, FORM_STR_RPL_ETRACE,
					   ...
					   class_name,
					   target_p->name,
					   target_p->username, target_p->realhost, ip,
					   target_p->info);
		}
	}
}
