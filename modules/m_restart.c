 ...
 *  USA
 *
 *  $Id: m_restart.c 371 2006-05-08 19:00:15Z jon $
 */
...
mo_restart(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NOPRIVS), me.name, source_p->name, "restart");
		return;
	}
	...
	ircsprintf(buf, "received RESTART command from %s", get_oper_name(source_p));
	server_die(buf, YES);
}
