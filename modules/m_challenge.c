 ...
 *  USA
 *
 *  $Id: m_challenge.c 459 2006-09-10 18:17:33Z jon $
 */
...
const char *_version = "$Revision: 459 $";
#endif
...
m_challenge(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(*parv[1] == '+')
	{
		...
		conf = find_exact_name_conf(OPER_TYPE,
					    source_p->localClient->auth_oper,
					    source_p->username, source_p->host, source_p->certfp);
		if(conf == NULL)
			conf = find_exact_name_conf(OPER_TYPE,
						    source_p->localClient->auth_oper, source_p->username,
						    source_p->realhost, source_p->certfp);
		if(conf == NULL)
			conf = find_exact_name_conf(OPER_TYPE,
						    source_p->localClient->auth_oper, source_p->username,
						    source_p->sockhost, source_p->certfp);
		if(conf == NULL)
		{
	...
	{
		sendto_one(source_p, form_str(ERR_NOOPERHOST), me.name, parv[0]);
		conf = find_exact_name_conf(OPER_TYPE, parv[1], NULL, NULL, NULL);
...
failed_challenge_notice(struct Client *source_p, const char *name, const char *reason)
{
	if(ConfigFileEntry.failed_oper_notice)
		sendto_realops_flags(UMODE_ALL, L_ALL, "Failed CHALLENGE attempt as %s "
				     ...
				     source_p->username, source_p->realhost, reason);
}
