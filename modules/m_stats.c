 ...
 *  USA
 *
 *  $Id: m_stats.c 371 2006-05-08 19:00:15Z jon $
 */
...
const char *_version = "$Revision: 371 $";
...
do_stats(struct Client *source_p, int parc, char **parv)
{
	...
	if(statchar == '\0')
	{
		sendto_one(source_p, form_str(RPL_ENDOFSTATS), from, to, '*');
		return;
	}
	...
	for(i = 0; stats_cmd_table[i].handler; i++)
	{
		if(stats_cmd_table[i].letter == statchar)
		{
			...
			{
				sendto_one(source_p, form_str(ERR_NOPRIVILEGES), from, to);
				break;
			}
	...
	sendto_one(source_p, form_str(RPL_ENDOFSTATS), from, to, statchar);
}
...
m_stats(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(RPL_LOAD2HI), from, to);
		return;
	}
	else
...
count_memory(struct Client *source_p)
{
	...
	channel_memory = dlink_list_length(&global_channel_list) * sizeof(struct Channel);
	...
	sendto_one(source_p, ":%s %d %s z :User aways %u(%d)",
		   me.name, RPL_STATSDEBUG, source_p->name, aways_counted, (int) away_memory);
	...
	sendto_one(source_p, ":%s %d %s z :Channels %lu(%lu) Topics %u(%d)",
		   ...
		   channel_memory, topic_count, topic_count * (TOPICLEN + 1 + USERHOST_REPLYLEN));

	sendto_one(source_p, ":%s %d %s z :Bans %u(%u)",
		   me.name, RPL_STATSDEBUG, source_p->name, channel_bans, channel_ban_memory);

	sendto_one(source_p, ":%s %d %s z :Exceptions %u(%u)",
		   me.name, RPL_STATSDEBUG, source_p->name, channel_except, channel_except_memory);

	sendto_one(source_p, ":%s %d %s z :Invex %u(%u)",
		   me.name, RPL_STATSDEBUG, source_p->name, channel_invex, channel_invex_memory);
	...
	total_channel_memory = channel_memory + channel_ban_memory +
		channel_users * sizeof(struct Membership) + channel_invites * sizeof(dlink_node);

	sendto_one(source_p, ":%s %d %s z :Safelist %u(%u)",
		   me.name, RPL_STATSDEBUG, source_p->name, safelist_count, safelist_memory);
...
****/
	...
	local_client_memory_used =
		local_client_count * (sizeof(struct Client) + sizeof(struct LocalUser));
...
stats_pending_glines(struct Client *source_p)
{
	...
	if(dlink_list_length(&pending_glines) > 0)
		sendto_one(source_p, ":%s NOTICE %s :Pending G-lines", from, to);
	...
	sendto_one(source_p, ":%s NOTICE %s :End of Pending G-lines", from, to);
#else
	sendto_one(source_p, ":%s NOTICE %s :This server does not support G-Line voting", from, to);
#endif /* GLINE VOTING */
}
...
stats_klines(struct Client *source_p)
{
	...
	else if((ConfigFileEntry.stats_k_oper_only == 1) && !IsOper(source_p))
	{
		...
		sendto_one(source_p, form_str(RPL_STATSKLINE), from,
			   to, "K", aconf->host, aconf->user, aconf->reason, aconf->oper_reason);
	}
	/* Theyre opered, or allowed to see all klines */
	else
	{
...
stats_servers(struct Client *source_p)
{
	...
	sendto_one(source_p, ":%s %d %s v :%d Server(s)", from, RPL_STATSDEBUG, to, j);
}
...
stats_ziplinks(struct Client *source_p)
{
	...
	sendto_one(source_p, ":%s %d %s Z :%u ziplink(s)", from, RPL_STATSDEBUG, to, sent_data);
}
...
stats_servlinks(struct Client *source_p)
{
	...
	DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		sendto_one(source_p, form_str(RPL_STATSLINKINFO),
			   ...
			   (CurrentTime >
			    target_p->since) ? (unsigned) (CurrentTime - target_p->since) : 0,
	...
	sendto_one(source_p, ":%s %d %s ? :Sent total : %7.2f %s",
		   from, RPL_STATSDEBUG, to, _GMKv(sendB), _GMKs(sendB));
	sendto_one(source_p, ":%s %d %s ? :Recv total : %7.2f %s",
		   from, RPL_STATSDEBUG, to, _GMKv(recvB), _GMKs(recvB));
...
stats_ltrace(struct Client *source_p, int parc, char *parv[])
{
	...
	if((name = parse_stats_args(parc, parv, &doall, &wilds)) != NULL)
	{
		...
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), from, to, "STATS");
}
	...
	DLINK_FOREACH(ptr, list->head)
	{
		     ...
		     !IsHandshake(target_p) && !IsConnecting(target_p))))
		{
			sendto_one(source_p, form_str(RPL_STATSLINKINFO),
				   ...
				   (unsigned) (CurrentTime - target_p->firsttime),
				   (CurrentTime >
				    target_p->since) ? (unsigned) (CurrentTime -
								   target_p->since) : 0,
