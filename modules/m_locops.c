 ...
 *  USA
 *
 *  $Id: m_locops.c 147 2005-12-26 02:52:10Z jon $
 */
...
ms_locops(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	sendto_server(client_p, NULL, NULL, CAP_CLUSTER, 0, 0, "LOCOPS %s :%s", parv[1], parv[2]);

	if(!IsClient(source_p) || !match(parv[1], me.name))
		return;

	if(find_matching_name_conf(ULINE_TYPE, source_p->servptr->name, "*", "*", SHARED_LOCOPS))
