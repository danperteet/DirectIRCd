 ...
 *  USA
 *
 *  $Id: m_testline.c 447 2006-09-04 18:50:02Z jon $
 */
...
const char *_version = "$Revision: 447 $";
#endif
...
mo_testline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
		...
		if(aconf != NULL)
		{
			...
			if(aconf->status & CONF_EXEMPTDLINE)
				...
				sendto_one(source_p, form_str(RPL_TESTLINE),
					   ...
					   IsConfTemporary(aconf) ? ((aconf->hold - CurrentTime) /
								     60) : 0L, aconf->host,
	...
	if(matches == 0)
		sendto_one(source_p, form_str(RPL_NOTESTLINE), me.name, source_p->name, parv1_copy);
}
 ...
 */
static void
mo_testgecos(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	struct ConfItem *conf = NULL;

	if(EmptyString(parv[1]))
	{
		sendto_one(source_p, ":%s NOTICE %s :usage: gecos", me.name, source_p->name);
		return;
	}
