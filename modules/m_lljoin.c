 ...
 *  USA
 *
 *  $Id: m_lljoin.c 403 2006-06-14 05:04:42Z jon $
 */
 ...
 */
static void
ms_lljoin(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	char *chname = NULL;
	char *nick = NULL;
	char *key = NULL;
	...
	if(uplink && !IsCapable(uplink, CAP_LL))
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "*** LLJOIN requested from non LL server %s", client_p->name);
		return;
	}
	...
	{
		sendto_one(target_p, form_str(ERR_CHANBANREASON), me.name, nick, chptr->chname,
	...
	if((i = can_join(target_p, chptr, key)))
	{
		if(i == ERR_NEEDREGGEDNICK)
			sendto_one(target_p, form_str(ERR_NEEDREGGEDNICK), me.name,
				   nick, chptr->chname, "join");
		else
			sendto_one(target_p, form_str(i), me.name, nick, chptr->chname);
