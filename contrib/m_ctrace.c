 ...
 *  USA
 *
 *  $Id: m_ctrace.c 131 2005-12-10 04:59:29Z jon $
 */
...
*/
static void
mo_ctrace(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	if(EmptyString(parv[1]))
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "CTRACE");
		return;
	}
...
do_ctrace(struct Client *source_p, int parc, char **parv)
{
	...
	sendto_one(source_p, form_str(RPL_ENDOFTRACE), me.name, parv[0], class_looking_for);
}
...
report_this_status(struct Client *source_p, struct Client *target_p)
{
	...
	case STAT_CLIENT:
		...
		if((IsOper(source_p) &&
		    (MyClient(source_p) || !IsInvisible(target_p))) || IsOper(target_p))
		{
