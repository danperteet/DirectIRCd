 ...
 *  USA
 *
 *  $Id: m_classlist.c 131 2005-12-10 04:59:29Z jon $
 */
 ...
 */
static void
mo_classlist(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	DLINK_FOREACH(ptr, class_items.head)
	{
		...
		if(match(parv[1], conf->name))
		{
			...
			sendto_one(source_p, ":%s NOTICE %s :%s %d",
				   me.name, source_p->name, conf->name, CurrUserCount(aclass));
			found_class = 1;
		}
	}
