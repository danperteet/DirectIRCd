 ...
 *
 *   $Id: m_operspy.c 485 2007-06-10 07:59:07Z jon $
 */
...
const char *_version = "$Revision: 485 $";
#endif
...
static void
ms_operspy(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
#ifdef OPERSPY_LOG
 ...
 */
static void
mo_operspy(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
...
operspy_mode(struct Client *client_p, int parc, char *parv[])
{
	...
	if((chptr_mode = hash_find_channel(parv[2])) == NULL)
	{
		 ...
		 * info.
		 */
		sendto_one(client_p, form_str(ERR_NOSUCHCHANNEL), me.name, client_p->name, parv[2]);
		return;
	}
...
operspy_topic(struct Client *client_p, int parc, char *parv[])
{
	...
	if(chptr_topic->topic == NULL)
		sendto_one(client_p, form_str(RPL_NOTOPIC), me.name, client_p->name, parv[2]);
	else
	{
...
operspy_who(struct Client *client_p, int parc, char *parv[])
{
	...
	if(mask != NULL)
	{
		...
		if(*mask == '\0')
		{
			sendto_one(client_p, form_str(RPL_ENDOFWHO), me.name, client_p->name, "*");
			return;
		}
	}
	else
	...
	if(IsChanPrefix(*mask))
	{
		...
		sendto_one(client_p, form_str(RPL_ENDOFWHO), me.name, client_p->name, mask);
		return;
	}
	...
	if((target_p_who = find_person(client_p, mask)) != NULL)
	{
		...
		ircsprintf(nuh, "%s!%s@%s %s", target_p_who->name,
			   target_p_who->username, target_p_who->host, target_p_who->servptr->name);
...
operspy_whois(struct Client *client_p, int parc, char *parv[])
{
	...
	{
		sendto_one(client_p, form_str(ERR_NOSUCHNICK), me.name, client_p->name, parv[2]);
		return;
	}
	...
	sendto_one(client_p, form_str(RPL_WHOISSERVER), me.name,
		   client_p->name, target_p->name, a2client_p->name, a2client_p->info);
	...
	if(buf[0])
		sendto_one(client_p, form_str(RPL_WHOISOPERATOR),
			   me.name, client_p->name, target_p->name, buf);

	if(MyConnect(target_p))
		sendto_one(client_p, form_str(RPL_WHOISIDLE), me.name,
			   client_p->name, target_p->name,
			   CurrentTime - target_p->localClient->last, target_p->firsttime);
	sendto_one(client_p, form_str(RPL_ENDOFWHOIS), me.name, client_p->name, parv[2]);
}

/* extensions for OPERSPY WHO */
static void
do_who(struct Client *source_p, struct Client *target_p, char *chname, const char *op_flags)
{
	char status[8];
	...
	DLINK_FOREACH(lp, global_client_list.head)
	{
		   ...
		   (MyClient(target_p) && match(mask, target_p->sockhost)))
		{
			if(dlink_list_length(&target_p->channel))
			{
				...
				chptr = ((struct Membership *) (target_p->channel.head->data))->
					chptr;
...
static void
do_who_on_channel(struct Client *source_p, struct Channel *chptr, char *chname)
{
	dlink_node *ptr;
...
operspy_log(struct Client *source_p, const char *command, const char *target)
{
	...
	nbytes = ircsprintf(linebuf, "[%s] OPERSPY %s %s %s\n",
			    smalldate(CurrentTime), get_oper_name(source_p), command, target);
