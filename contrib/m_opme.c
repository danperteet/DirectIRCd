 ...
 *  USA
 *
 *  $Id: m_opme.c 131 2005-12-10 04:59:29Z jon $
 */
...
mo_opme(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	sendto_server(NULL, source_p, chptr, CAP_TS6, NOCAPS, NOFLAGS,
		      ":%s SJOIN %lu %s + :@%s",
		      me.id, (unsigned long) chptr->channelts, chptr->chname, ID(source_p));
	sendto_server(NULL, source_p, chptr, NOCAPS, CAP_TS6, NOFLAGS,
		      ":%s SJOIN %lu %s + :@%s",
		      me.name, (unsigned long) chptr->channelts, chptr->chname, source_p->name);
