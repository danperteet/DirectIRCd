 ...
 *
 *   $Id: io.c 131 2005-12-10 04:59:29Z jon $
 */
...
process_recvq(struct ctrl_command *cmd)
{
	...
	if(in_state.zip)
	{
		...
		{
			if((ret = inflate(&in_state.zip_state.stream, Z_NO_FLUSH)) != Z_OK)
...
read_data(void)
{
	...
	while ((ret = check_error(read(LOCAL_R.fd, buf, READLEN), IO_READ, LOCAL_R.fd)))
	{
		...
		if(out_state.zip)
		{
			...
			if(!(ret2 = deflate(&out_state.zip_state.stream, Z_PARTIAL_FLUSH)) == Z_OK)
				send_error
					("error compressing outgoing data - deflate returned %d (%s)",
					 ret2, zError(ret2));
...
read_net(void)
{
	...
	while ((ret = check_error(read(REMOTE_R.fd, buf, READLEN), IO_READ, REMOTE_R.fd)))
	{
		...
		if(in_state.zip)
		{
			...
			while (in_state.zip_state.stream.avail_in)
			{
				if((ret2 = inflate(&in_state.zip_state.stream, Z_NO_FLUSH)) != Z_OK)
