 ...
 *  USA
 *
 *  $Id: hostmask.c 425 2006-07-17 02:51:38Z jon $
 */
...
try_parse_v4_netmask(const char *text, struct irc_ssaddr *addr, int *b)
{
	...
	for(n = bits / 8 + (bits % 8 ? 1 : 0); n < 4; n++)
		addb[n] = 0;
	if(addr)
		v4->sin_addr.s_addr = htonl(addb[0] << 24 | addb[1] << 16 | addb[2] << 8 | addb[3]);
	if(b != NULL)
		*b = bits;
	return HM_IPV4;
}
...
match_ipv6(struct irc_ssaddr *addr, struct irc_ssaddr *mask, int bits)
{
	...
	if((m = bits % 8) == 0)
		return -1;
	if((v6->sin6_addr.s6_addr[n] & ~((1 << (8 - m)) - 1)) == v6mask->sin6_addr.s6_addr[n])
		return -1;
	return 0;
}
#endif
			...
			for(b = 128; b >= 0; b -= 16)
			{
				for(arec = atable[hash_ipv6(addr, b)]; arec; arec = arec->next)
					if(arec->type == (type & ~0x1) &&
					   ...
					   (IsNeedPassword(arec->aconf)
					    || arec->aconf->passwd == NULL
...
struct AccessItem *
find_gline_conf(const char *host, const char *user, struct irc_ssaddr *ip, int aftype)
{
	struct AccessItem *eline;

	eline = find_conf_by_address(host, ip, CONF_EXEMPTKLINE, aftype, user, NULL, NULL);
	if(eline != NULL)
		return (eline);
		...
		struct irc_ssaddr *ip, int aftype)
{
	...
	return (find_conf_by_address(host, ip, CONF_KILL, aftype, user, NULL, certfp));
}
...
find_dline_conf(struct irc_ssaddr *addr, int aftype)
{
	...
	eline = find_conf_by_address(NULL, addr, CONF_EXEMPTDLINE | 1, aftype, NULL, NULL, NULL);
	if(eline != NULL)
		return (eline);
			...
			if(arec->type == CONF_CLIENT)
			{
				...
				if(ConfigFileEntry.hide_spoof_ips)
					sendto_one(client_p, form_str(RPL_STATSILINE), me.name,
						   ...
						   aconf->class_ptr ? aconf->class_ptr->
						   name : "<default>");
