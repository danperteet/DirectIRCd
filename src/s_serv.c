 ...
 *  USA
 *
 *  $Id: s_serv.c 223 2006-01-17 22:24:31Z jon $
 */
...
write_links_file(void *notused)
{
	...
	DLINK_FOREACH(ptr, global_serv_list.head)
	{
		 ...
		 */
		assert(strlen(target_p->name) + strlen(me.name) + 6 + strlen(p) <= MESSAGELINELEN);
	    ...
	    int server, int parc, char *parv[])
{
	...
	if(parc <= server || EmptyString(parv[server]) ||
	   ...
	   match(parv[server], me.name) || !strcmp(parv[server], me.id))
		return (HUNTED_ISME);
	...
	if(target_p == NULL)
	{
		...
		else
		{
			DLINK_FOREACH(ptr, global_client_list.head)
			{
				...
				{
					if(target_tmp->from == source_p->from
					   && !MyConnect(target_tmp))
						continue;
					target_p = ptr->data;
		...
		if(CurrUserCount(cltmp) < MaxTotal(cltmp))
		{
			...
			if(ConfigServerHide.hide_server_ips)
				sendto_realops_flags(UMODE_ALL, L_ALL,
						     "Connection to %s activated.", conf->name);
			else
				sendto_realops_flags(UMODE_ALL, L_ALL,
						     "Connection to %s[%s] activated.", conf->name,
						     aconf->host);
...
check_server(const char *name, struct Client *client_p, int cryptlink)
{
	...
	DLINK_FOREACH(ptr, server_items.head)
	{
		...
		if(match(aconf->host, client_p->host) || match(aconf->host, client_p->sockhost))
		{
			error = -2;
#ifdef HAVE_LIBCRYPTO
...
#endif /* HAVE_LIBCRYPTO */
			{
				...
				if(IsConfEncrypted(aconf))
				{
					if(strcmp(aconf->passwd,
						  (const char *) crypt(client_p->localClient->
								       passwd, aconf->passwd)) == 0)
						server_conf = conf;
				}
				else
				{
...
show_capabilities(struct Client *target_p)
{
	...
	if(IsCapable(target_p, CAP_ENC) &&
	   target_p->localClient->in_cipher && target_p->localClient->out_cipher)
		t += ircsprintf(t, "ENC:%s ", target_p->localClient->in_cipher->name);
#endif
	*(t - 1) = '\0';
...
server_estab(struct Client *client_p)
{
	...
	if(IsUnknown(client_p) && !IsConfCryptLink(aconf))
	{
		...
		if(!EmptyString(aconf->spasswd))
		{
			...
			if(me.id[0] != '\0')	/* Send TS 6 form only if id */
				...
				sendto_one(client_p, "PASS %s TS 5", aconf->spasswd);
		}
	...
	sendto_one(client_p, "SVINFO %d %d 0 :%lu",
		   (me.id[0] ? TS_CURRENT : 5), TS_MIN, (unsigned long) CurrentTime);
	...
	/* If the server matches a services shared{} entry then mark it as such - ThaPrince */
	if(find_matching_name_conf(ULINE_TYPE, client_p->name, "*", "*", SHARED_SERVICES))
		...
		if(IsCapable(client_p, CAP_TS6))
		{
			if(HasID(target_p))
				sendto_one(client_p, ":%s SID %s %d %s :%s%s",
					   ID(target_p->servptr), target_p->name,
...
burst_all(struct Client *client_p)
{
	...
	DLINK_FOREACH(ptr, global_channel_list.head)
	{
		...
		if((dlink_list_length(&chptr->members) != 0) || PersistChannel(chptr))
		{
			...
			if(IsCapable(client_p, CAP_TBURST) || IsCapable(client_p, CAP_TB))
...
serv_connect_callback(fde_t * fd, int status, void *data)
{
	...
	memcpy(&client_p->localClient->ip, &fd->connect.hostaddr, sizeof(struct irc_ssaddr));
	/* Check the status */
	if(status != COMM_OK)
	{
	...
	/* Get the C/N lines */
	conf = find_conf_name(&client_p->localClient->confs, client_p->name, SERVER_TYPE);
	if(conf == NULL)
	{
		sendto_realops_flags(UMODE_ALL, L_ADMIN,
				     "Lost connect{} block for %s", get_client_name(client_p,
										    HIDE_IP));
		sendto_realops_flags(UMODE_ALL, L_OPER, "Lost connect{} block for %s",
				     get_client_name(client_p, MASK_IP));
	...
	if(!EmptyString(aconf->spasswd) && (me.id[0] != '\0'))
		...
		sendto_one(client_p, "PASS %s TS %d %s", aconf->spasswd, TS_CURRENT, me.id);
	else
		sendto_one(client_p, "PASS %s TS 5", aconf->spasswd);
	...
	sendto_one(client_p, "SERVER %s 1 :%s%s",
		   my_name_for_link(conf), ConfigServerHide.hidden ? "(H) " : "", me.info);
