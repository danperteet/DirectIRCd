 ...
 *  USA
 *
 *  $Id: hash.c 283 2006-02-13 04:24:55Z jon $
 */
...
find_server(const char *name)
{
	...
	{
		if((!IsServer(client_p) && !IsMe(client_p)) || irccmp(name, client_p->name))
		{
			struct Client *prev;
 ...
 */
void
count_user_host(const char *user, const char *host, int *global_p, int *local_p, int *icount_p)
{
	dlink_node *ptr;
...
list_allow_channel(const char *chname, struct ListTask *lt)
{
	...
	DLINK_FOREACH(dl, lt->show_mask.head) if(!match_chan(dl->data, chname))
		return 0;

	DLINK_FOREACH(dl, lt->hide_mask.head) if(match_chan(dl->data, chname))
		return 0;
		 ...
		 struct ListTask *list_task, int remote_request)
{
	...
	if((remote_request && chptr->chname[0] == '&') ||
	   (SecretChannel(chptr) && !IsMember(source_p, chptr) && !IsAdmin(source_p)))
		return;
	...
	sendto_one(source_p, form_str(RPL_LIST), me.name, source_p->name,
		   ...
		   modes, chptr->topic == NULL ? "" : chptr->topic);
}
