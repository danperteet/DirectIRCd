 ...
 *  USA
 *
 *  $Id: s_conf.c 509 2007-07-16 20:19:03Z jon $
 */
...
{
	void *conf;
	conf = (void *) ((unsigned long) aconf + (unsigned long) sizeof(struct ConfItem));
	return (conf);
}
...
conf_dns_callback(void *vptr, struct DNSReply *reply)
{
	...
	{
		ilog(L_NOTICE, "Host not found: %s, ignoring connect{} block", aconf->host);
...
make_conf_item(ConfType type)
{
	...
	case OPER_TYPE:
	case SERVER_TYPE:
		conf = MyMalloc(sizeof(struct ConfItem) + sizeof(struct AccessItem));
	...
	case CLASS_TYPE:
		conf = MyMalloc(sizeof(struct ConfItem) + sizeof(struct ClassItem));
	...
	case ULINE_TYPE:
		...
		DLINK_FOREACH(ptr, cluster_items.head)
		{
			...
			*p++ = 'C';
			...
			sendto_one(source_p, form_str(RPL_STATSULINE),
				   me.name, source_p->name, conf->name, "*", "*", buf);
		}
	...
	case CLASS_TYPE:
		DLINK_FOREACH(ptr, class_items.head)
		{
			...
			sendto_one(source_p, form_str(RPL_STATSYLINE),
				   ...
				   ConFreq(classitem), MaxTotal(classitem), MaxSendq(classitem),
...
check_client(va_list args)
{
	...
	if((i = verify_access(source_p, username)))
		ilog(L_INFO, "Access denied: %s[%s]", source_p->name, source_p->sockhost);
	...
	case TOO_MANY:
		sendto_realops_flags(UMODE_FULL, L_ALL,
				     ...
				     get_client_name(source_p, SHOW_IP), source_p->sockhost);
	...
	case I_LINE_FULL:
		sendto_realops_flags(UMODE_FULL, L_ALL,
				     ...
				     get_client_name(source_p, SHOW_IP), source_p->sockhost);
		ilog(L_INFO, "Too many connections from %s.", get_client_name(source_p, SHOW_IP));
		ServerStats->is_ref++;
		exit_client(source_p, &me, "No more connections allowed in your connection class");
		break;
	...
	case NOT_AUTHORIZED:
		{
			...
			irc_getnameinfo((struct sockaddr *) &source_p->localClient->ip,
					source_p->localClient->ip.ss_len, ipaddr, HOSTIPLEN, NULL,
					0, NI_NUMERICHOST);
			...
			if(REJECT_HOLD_TIME > 0)
			{
				sendto_one(source_p,
					   ...
					   me.name, source_p->name);
				source_p->localClient->reject_delay =
					CurrentTime + REJECT_HOLD_TIME;
		...
		if(IsConfClient(aconf) && !rkconf)
		{
			...
			if(IsConfRedir(aconf))
			{
				sendto_one(client_p, form_str(RPL_REDIR),
					   ...
					   conf->name ? conf->name : "", aconf->port);
			...
			|| (ConfigFileEntry.glines && IsConfGline(aconf)))
		{
			/* XXX */
...
attach_iline(struct Client *client_p, struct ConfItem *conf)
{
	...
	count_user_host(client_p->username, client_p->host, &global, &local, &ident);
	...
	if(MaxTotal(aclass) != 0 && CurrUserCount(aclass) >= MaxTotal(aclass))
		...
		a_limit_reached = 1;
	else if(MaxIdent(aclass) != 0 && ident >= MaxIdent(aclass) && client_p->username[0] != '~')
		a_limit_reached = 1;
			...
			case SERVER_TYPE:
				...
				if((aclass_conf = ClassPtr(aconf)) != NULL)
				{
					...
					if(conf->type == CLIENT_TYPE)
						remove_from_cidr_check(&client_p->localClient->ip,
								       aclass);
...
attach_conf(struct Client *client_p, struct ConfItem *conf)
{
	...
	if(conf->type == CLIENT_TYPE || conf->type == SERVER_TYPE || conf->type == OPER_TYPE)
	{
 ...
 */
struct ConfItem *
find_conf_exact(ConfType type, const char *name, const char *user, const char *host)
{
	dlink_node *ptr;
	dlink_list *list_p;
	struct ConfItem *conf = NULL;
...
find_conf_name(dlink_list * list, const char *name, ConfType type)
{
	...
	DLINK_FOREACH(ptr, list->head)
	{
		...
		if(conf->type == type)
		{
			if(conf->name && (irccmp(conf->name, name) == 0 || match(conf->name, name)))
				return conf;
		}
	}
...
set_default_conf(void)
{
	...
	ConfigChannel.cycle_on_hostchange = NO;
	...
	ConfigFileEntry.anti_spam_connect_numeric = NO;
	...
	ConfigFileEntry.oper_umodes = UMODE_BOTS | UMODE_LOCOPS | UMODE_SERVNOTICE | UMODE_OPERWALL | UMODE_WALLOP;	/* XXX */
...
lookup_confhost(struct ConfItem *conf)
{
	...
	{
		ilog(L_ERROR, "Host/server name error: (%s) (%s)", aconf->host, conf->name);
		return;
	}
...
find_gline(struct Client *client_p)
{
	...
	aconf = find_gline_conf(client_p->realhost, client_p->username,
...
expire_tklines(dlink_list * tklist)
{
	...
	{
		conf = ptr->data;
		if(conf->type == GLINE_TYPE || conf->type == KLINE_TYPE || conf->type == DLINE_TYPE)
		{
			aconf = (struct AccessItem *) map_to_conf(conf);
		...
		else if(conf->type == XLINE_TYPE || conf->type == RXLINE_TYPE)
		{
			...
			if(xconf->hold <= CurrentTime)
			{
				if(ConfigFileEntry.tkline_expire_notices)
					sendto_realops_flags(UMODE_ALL, L_ALL,
							     "Temporary X-line for [%s] %sexpired",
							     conf->name,
							     conf->type ==
							     RXLINE_TYPE ? "(REGEX) " : "");
				dlinkDelete(ptr, tklist);
...
read_conf_files(int cold)
{
	...
	ircsprintf(targmax_buf, "ACCEPT:,KICK:1,LIST:1,NAMES:1,NOTICE:%d,PRIVMSG:%d,WHOIS:1",
		   ConfigFileEntry.max_targets, ConfigFileEntry.max_targets);
	add_isupport("TARGMAX", targmax_buf, -1);
	...
	for(; *iterator != NULL; iterator++)
	{
		DLINK_FOREACH_SAFE(ptr, next_ptr, (*iterator)->head)
		{
			...
			else if(conf->type == CLIENT_TYPE)
			{
				...
				conf->type == RXLINE_TYPE || conf->type == RKLINE_TYPE)
			{
...
get_client_class(struct Client *target_p)
{
	...
	if(target_p != NULL && !IsMe(target_p) && target_p->localClient->confs.head != NULL)
	{
...
conf_add_server(struct ConfItem *conf, const char *class_name)
{
	...
	{
		sendto_realops_flags(UMODE_ALL, L_ALL, "Bad connect block, name %s", conf->name);
...
yyerror(const char *msg)
{
	...
	ilog(L_WARN, "\"%s\", line %u: %s: %s", conffilebuf, lineno + 1, msg, newlinebuf);
}
	    ...
	    char **target_server, char **reason)
{
	...
	if(parc == 0)
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name, source_p->name, cmd);
		return -1;
	}
	...
	else if((parse_flags & AWILD) && !valid_wild_card(source_p, YES, 1, *up_p))
		return -1;
	       ...
	       char *luser, char *lhost, unsigned int flags)
{
	...
	else if(!(flags & NOUSERLOOKUP))
	{
		...
		if((target_p = find_chasing(source_p, source_p, user_host_or_nick, NULL)) == NULL)
			return 0;
 ...
 */
static void
flags_to_ascii(unsigned int flags, const unsigned int bit_table[], char *p, int lowerit)
{
	unsigned int mask = 1;
	int i = 0;
 ...
 */
static int
cidr_limit_reached(int over_rule, struct irc_ssaddr *ip, struct ClassItem *aclass)
{
	dlink_node *ptr = NULL;
	...
	if(ip->ss.ss_family == AF_INET)
	{
		...
		DLINK_FOREACH(ptr, aclass->list_ipv4.head)
		{
			...
			{
				if(!over_rule
				   && (cidr->number_on_this_cidr >= NumberPerCidr(aclass)))
					return -1;
