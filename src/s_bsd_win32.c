 ...
 *  USA
 *
 *  $Id: s_bsd_win32.c 147 2005-12-26 02:52:10Z jon $
 */
	...
	case WM_DNS:
		{
				...
				if(((struct DNSQuery *) ptr->data)->handle == wParam)
			{
				...
				if(WSAGETASYNCERROR(lParam) == 0)
				{
					...
					{
					case AF_INET:
						memcpy(&((struct sockaddr_in *) &reply->addr)->
						       sin_addr, h->h_addr_list[0], h->h_length);
						break;

#ifdef IPV6
					case AF_INET6:
						memcpy(&((struct sockaddr_in6 *) &reply->addr)->
						       sin6_addr, h->h_addr_list[0], h->h_length);
						break;
#endif
 ...
 */
void
comm_setselect(fde_t * F, unsigned int type, PF * handler, void *client_data, time_t timeout)
{
	int new_events = 0;
...
{
	query->handle = WSAAsyncGetHostByAddr(wndhandle, WM_DNS,
#ifdef IPV6
					      addr.ss.ss_family ==
...
#endif
		sizeof(struct in_addr), addr->ss.ss_family, query->reply, sizeof(query->reply));
...
int uname(struct utsname *uts)
{
	...
	if(osver.szCSDVersion[0] != '\0' &&
	   (strlen(osver.szCSDVersion) + strlen(uts->version) + 1) < sizeof(uts->version))
	{
		strcat(uts->version, " ");
