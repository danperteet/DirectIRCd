 ...
 *  USA
 *
 *  $Id: s_user.c 493 2007-07-07 17:45:14Z jon $
 */
...
show_lusers(struct Client *source_p)
{
	...
	if(!ConfigServerHide.hide_servers || IsOper(source_p))
	{
		sendto_one(source_p, form_str(RPL_LUSERME), from, to, Count.local, Count.myserver);
		sendto_one(source_p, form_str(RPL_LOCALUSERS),
			   from, to, Count.local, Count.max_loc, Count.local, Count.max_loc);
	}
	else
	{
		sendto_one(source_p, form_str(RPL_LUSERME), from, to, Count.total, 0);
		sendto_one(source_p, form_str(RPL_LOCALUSERS),
			   from, to, Count.total, Count.max_tot, Count.total, Count.max_tot);
	}
		    ...
		    const char *nick, const char *username)
{
	...
	if(ConfigFileEntry.ping_cookie)
	{
		if(!IsPingSent(source_p) && source_p->localClient->random_ping == 0)
		{
			...
			sendto_one(source_p, "PING :%lu", source_p->localClient->random_ping);
	   ...
	   (Count.local >= ServerInfo.max_clients && !IsExemptLimits(source_p)))
	{
		sendto_realops_flags(UMODE_FULL, L_ALL,
				     "Too many clients, rejecting %s[%s].", nick,
				     source_p->realhost);
		ServerStats->is_ref++;
...
introduce_client(struct Client *client_p, struct Client *source_p)
{
	 ...
	 */
	if(!ServerInfo.hub && uplink && IsCapable(uplink, CAP_LL) && client_p != uplink)
	{
...
report_and_set_user_flags(struct Client *source_p, const struct AccessItem *aconf)
{
	...
	if(IsConfCanFlood(aconf))
	{
		...
		sendto_one(source_p, ":%s NOTICE %s :*** You are exempt from flood "
			   "protection, aren't you fearsome.", me.name, source_p->name);
	}
}
			...
			case 'o':
				...
				else
				{
					...
					if(MyConnect(source_p))
					{
						...
						if((dm =
						    dlinkFindDelete(&oper_list, source_p)) != NULL)
							free_dlink_node(dm);
					}
				}
			...
			default:
				if((flag = user_modes[(unsigned char) *m]))
				{
					   ...
					   (ConfigFileEntry.oper_only_umodes & flag))
					{
						...
						execute_callback(umode_cb, client_p, source_p, what,
								 flag);
				}
				else
				{
 ...
 */
void
send_umode_out(struct Client *client_p, struct Client *source_p, unsigned int old)
{
...
user_welcome(struct Client *source_p)
{
	...
	sendto_one(source_p, form_str(RPL_YOURHOST), me.name, source_p->name,
		   get_listener_name(source_p->localClient->listener), ircd_version, serno);
	sendto_one(source_p, form_str(RPL_CREATED), me.name, source_p->name, built_date);
	sendto_one(source_p, form_str(RPL_MYINFO),
		   me.name, source_p->name, me.name, ircd_version, serno, umode_buffer);
	show_isupport(source_p);

	if(source_p->id[0] != '\0')
		sendto_one(source_p, form_str(RPL_YOURID), me.name, source_p->name, source_p->id);
...
#if defined(CHANAQ) && defined(HALFOPS)
	add_isupport("PREFIX", "(qaohv)~&@%+", -1);
	add_isupport("STATUSMSG", "~&@%+", -1);
#elif defined(CHANAQ) && !defined(HALFOPS)
	add_isupport("PREFIX", "(qaov)~&@+", -1);
	add_isupport("STATUSMSG", "~&@+", -1);
#elif !defined(CHANAQ) && defined(HALFOPS)
