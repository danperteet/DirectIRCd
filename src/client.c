 ...
 *  USA
 *
 *  $Id: client.c 469 2007-03-17 02:59:20Z jon $
 */
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, list->head)
	{
		...
		if(GlobalSetOptions.idletime && IsClient(client_p))
		{
			if(!IsExemptKline(client_p) && !IsOper(client_p) &&
			   ...
			   ((CurrentTime - client_p->localClient->last) >
			    GlobalSetOptions.idletime))
			{
				...
				DupString(aconf->host, client_p->realhost);
...
find_chasing(struct Client *client_p, struct Client *source_p, const char *user, int *chasing)
{
	...
	if((who = get_history(user, (time_t) ConfigFileEntry.kill_chase_time_limit)) == NULL)
	{
...
get_client_name(struct Client *client, int showip)
{
	...
	case HIDE_REALHOST:
		...
		break;
	default:
		ircsprintf(nbuf, "%s[%s@%s]", client->name, client->username, client->realhost);
	}
		   ...
		   const char *splitstr, const char *myname)
{
	...
	if(!hidden && ((source_p == original_source_p && to != from) || !IsCapable(to, CAP_QS)))
	{
		  ...
		  const char *comment, const char *splitstr)
{
	...
	DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		recurse_send_quits(source_p, source_p, from, to, comment, splitstr, myname);
	}
	...
	if(MyConnect(source_p))
	{
		...
		else if(IsClient(source_p))
		{
			...
			sendto_realops_flags(UMODE_CCONN, L_ALL,
					     "Client exiting: %s (%s@%s) [%s] [%s]", source_p->name,
			...
			sendto_realops_flags(UMODE_CCONN_FULL, L_ALL,
					     ...
					     ConfigFileEntry.hide_spoof_ips && IsIPSpoof(source_p) ?
					     "255.255.255.255" : source_p->sockhost, comment);
		}
 ...
 */
void
set_initial_nick(struct Client *client_p, struct Client *source_p, const char *nick)
{
	char buf[USERLEN + 1];
...
{
	if(ConfigChannel.quiet_on_ban && !IsOper(source_p))
	{
		dlink_node *lp = NULL, *next_lp = NULL;

		DLINK_FOREACH_SAFE(lp, next_lp, source_p->channel.head)
		{
			struct Channel *chptr = ((struct Membership *) lp->data)->chptr;

			if(can_send(chptr, source_p, NULL, NULL, NO) < 1)
			{
	   ...
	   !ConfigFileEntry.anti_nick_flood || (IsOper(source_p) && ConfigFileEntry.no_oper_flood))
	{
		if(irccmp(source_p->name, nick))
		{
			...
			if(IsRegNick(source_p))
			{
				unsigned int oldumodes;
				char umodebuf[IRCD_BUFSIZE];

				oldumodes = source_p->umodes;
				ClearRegNick(source_p);
				send_umode(source_p, source_p, oldumodes, 0xffffffff, umodebuf);
			}
		}
